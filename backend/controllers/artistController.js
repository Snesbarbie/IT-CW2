...
const crudDayThree = async (req, res) => {
  ...
  // Get day one info
const crudDayFour = async (req, res) => {

  const artistModels = await artistModel.find({day: 4}).sort({createdAt: -1})

  res.status(200).json(artistModels)

  }
  // Get day one info
const crudDayFive = async (req, res) => {

  const artistModels = await artistModel.find({day: 5}).sort({createdAt: -1})
